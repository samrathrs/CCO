 ...
 * @version $Rivision:$
 */
public class PasswordPolicyNotificationCallback implements NotificationCallback {
	private static Logger lgr = Logger
			.getLogger(PasswordPolicyNotificationCallback.class.getName());
	...
	public void entityChanged(NotificationServerMessage message) {
		if (lgr.isLoggable(Level.INFO)) {
			lgr.info("Got a password policy changed/added/removed event.");
		}
		...
		//No need to notify TPG's as AIM is the only consumer of this info
		
		// Notify TPGs with the change
//		ACGNotificatonDelegator delegator = new ACGNotificatonDelegator(
//				PasswordPolicyNotificationCallback.class.getName());
