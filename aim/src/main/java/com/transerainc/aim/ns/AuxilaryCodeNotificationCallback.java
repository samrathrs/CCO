 ...
 * @version $Rivision:$
 */
public class AuxilaryCodeNotificationCallback implements NotificationCallback {
	private static Logger lgr = Logger
			.getLogger(AuxilaryCodeNotificationCallback.class.getName());
	...
	public void entityChanged(NotificationServerMessage message) {
		if (lgr.isLoggable(Level.INFO)) {
			lgr.info("Got an AuxCode changed/added/removed event.");
		}
		...
		ACGNotificatonDelegator delegator = new ACGNotificatonDelegator(
				AuxilaryCodeNotificationCallback.class.getName());
