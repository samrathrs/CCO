 ...
 * @version $Rivision:$
 */
public class VirtualTeamNotificationCallback implements NotificationCallback {
	private static Logger lgr = Logger
			.getLogger(VirtualTeamNotificationCallback.class.getName());
	...
	public void entityChanged(NotificationServerMessage message) {
		if (lgr.isLoggable(Level.INFO)) {
			lgr.info("Got a VirtualTeam changed/added/removed event.");
		}
		...
		ACGNotificatonDelegator delegator = new ACGNotificatonDelegator(
				VirtualTeamNotificationCallback.class.getName());
