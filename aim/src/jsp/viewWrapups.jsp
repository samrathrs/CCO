...
<%
	...
	WrapupHandler wHandler = provMgr.getWrapupHandler();

	List<AuxilaryCode> acList = wHandler.getWrapupsForTenant(tenantId);
...
<ul>
	<li><input type="submit" name="reload" value="Reload"><a
		href="viewWrapups.jsp?tenantId=<%=tenantId %>" rel="dropmenu1"><%=tenantId%>.
	<%=tenantName%></a></li>
</ul>
</div>
...
%> 	<a href="viewWrapups.jsp?tenantId=<%=tenant.getTenantId() %>"> <%=String.format("%1$4s.", tenant.getTenantId()).replaceAll(" ", "&nbsp;")%><%=tName%></a>
<%
	}
%>
</div>
...
<table id="b" class="mytable">
	<caption><span id="desc">Wrapup Codes For tenant <%=tenantName%></span></caption>
	<tr>
		<th>&nbsp;</th>
		<th>Id</th>
		<th>Name</th>
